...
use DagaSmart\BizAdmin\Support\CodeGenerator\ControllerGenerator;
...
EOF;
        ...
        AdminApiService::make()->query()->where('enabled', 1)->get()->map(function ($item) use (&$routes) {
            ...
            $routes .= <<<EOF
    // {$item->title}
    \$router->{$item->method}('{$_route}', [\DagaSmart\BizAdmin\Controllers\AdminApiController::class, 'index']);
