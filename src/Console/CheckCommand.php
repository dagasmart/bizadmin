...
class CheckCommand extends Command
{
    ...
    {
        try {
            $latest = file_get_contents('https://gitee.com/api/v5/repos/dagasmart/bizadmin/tags?sort=updated&direction=desc&page=1&per_page=1');
            $latest = json_decode($latest, true)[0]['name'];

            $now = Composer::getVersion('dagasmart/bizadmin');

            if ($now !== $latest) {
                $this->echoWarn(
                    "BizAdmin 最新版本 [{$latest}]，当前版本 [{$now}]，请及时更新",
                    'BizAdmin latest version {$latest}, current version {$now}, please update it immediately'
                );
            }
        } catch (\Throwable $e) {
        }
