...
if (!function_exists('admin_url')) {
    ...
    {
        $prefix = $needPrefix ? '/' . \DagaSmart\BizAdmin\Admin::config('admin.route.prefix') : '';
...
if (!function_exists('admin_resource_full_path')) {
    function admin_resource_full_path($path, $server = null)
    {
        ...
        if (url()->isValidUrl($path) || mb_strpos($path, 'data:image') === 0) {
            ...
            $disk = \DagaSmart\BizAdmin\Admin::config('admin.upload.disk');
        ...
        $scheme = 'http:';
        if (\DagaSmart\BizAdmin\Admin::config('admin.https', false)) {
            $scheme = 'https:';
        }
...
if (!function_exists('amis')) {
     ...
     * @param $type
     *
     * @return \DagaSmart\BizAdmin\Renderers\Amis|\DagaSmart\BizAdmin\Renderers\Component
     */
    function amis($type = null)
    {
        if (filled($type)) {
            return \DagaSmart\BizAdmin\Renderers\Component::make()->setType($type);
        }

        return \DagaSmart\BizAdmin\Renderers\Amis::make();
    }
}

if (!function_exists('amisMake')) {
    /**
     * @return \DagaSmart\BizAdmin\Renderers\Amis
     * @deprecated
     */
    function amisMake()
    {
...
if (!function_exists('file_upload_handle')) {
    ...
    {
        $storage = \Illuminate\Support\Facades\Storage::disk(\DagaSmart\BizAdmin\Admin::config('admin.upload.disk'));
...
if (!function_exists('settings')) {
    function settings()
    {
        return \DagaSmart\BizAdmin\Services\AdminSettingService::make();
    }
}
...
if (!function_exists('admin_user')) {
    function admin_user()
    {
        return \DagaSmart\BizAdmin\Admin::user();
    }
}
...
if (!function_exists('admin_abort')) {
    ...
    {
        throw new \DagaSmart\BizAdmin\Exceptions\AdminException($message, $data, $doNotDisplayToast);
    }
...
if (!function_exists('admin_pages')) {
    ...
    {
        return \DagaSmart\BizAdmin\Services\AdminPageService::make()->get($sign);
    }
}
...
if (!function_exists('admin_pipeline')) {
    ...
    {
        return \DagaSmart\BizAdmin\Support\Pipeline::handle($passable);
    }
}
